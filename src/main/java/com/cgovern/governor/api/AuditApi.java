...
public interface AuditApi {
	...
	@ApiOperation(value = "Add a new pet to the store", nickname = "retrieveAudit", notes = "", response = Audit.class, tags = {
			"audit", })
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Invalid input", response = Audit.class),
			@ApiResponse(code = 400, message = "Audit Not Found") })
	@RequestMapping(value = "/audit/{auditId}", produces = { "application/json" }, method = RequestMethod.GET)
	ResponseEntity<Audit> retrieveAudit(
	...
	@ApiResponses(value = { @ApiResponse(code = 204, message = "Accepted"),
			@ApiResponse(code = 404, message = "Audit not found") })
	@RequestMapping(value = "/audit/{auditId}", produces = { "application/json" }, consumes = {
			"application/json" }, method = RequestMethod.PUT)
	ResponseEntity<Void> updateAudit(
			@ApiParam(value = "AuditId object that needs to retrieved", required = true) @PathVariable("auditId") String auditId,
